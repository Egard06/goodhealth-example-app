...
Future<List<RegisPoli>> fetchRegisPolis() async {
  ...
  final response = await http.get(
    Uri.parse(
      'http://10.52.53.134/goodhealth/regis_poli/index.php?id_pasien=$idPasien',
    ),
  );
...
Future regisPoliCreate(RegisPoli regisPoli) async {
  ...
  try {
    final response = await http.post(
      Uri.parse('http://10.52.53.134/goodhealth/regis_poli/create.php'),
...
Future deleteRegisPoli(id) async {
  ...
  final response = await http.get(
    Uri.parse('http://10.52.53.134/goodhealth/regis_poli/delete.php?id=$id'),
  );
