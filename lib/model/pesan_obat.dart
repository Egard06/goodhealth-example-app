...
import 'package:good_healt/util/session.dart';
import 'pasien.dart';
...
class PesanObat {
  ...
  factory PesanObat.fromJson(Map<String, dynamic> json) {
    return PesanObat(
      ...
      isSelesai: (json['is_selesai'].toString() == "1") ? true : false,
    );
  }
}
...
Future<List<PesanObat>> fetchPesanObats({isSelesai}) async {
  ...
  final response = await http.get(
    Uri.parse(
      'http://10.52.53.134/goodhealth/pesan_obat/index.php?id_pasien=$idPasien&is_selesai=$isSelesai',
    ),
  );
...
/*Future<List<PesanObat>> fetchPesanObats({isSelesai}) async {
  ...
  final response = await http.get(Uri.parse('http://192.168.18.41/goodhealth/pesan_obat/index.php?id_pasien=$idPasien')); 
...
Future pesanObatCreate(PesanObat pesanObat) async {
  ...
  try {
    final response = await http.post(
      Uri.parse('http://10.52.53.134/goodhealth/pesan_obat/create.php'),
...
Future deletePesanObat(id) async {
  ...
  final response = await http.get(
    Uri.parse('http://10.52.53.134/goodhealth/pesan_obat/delete.php?id=$id"'),
  );
...
Future updatePesanObat(id) async {
  ...
  final response = await http.get(
    Uri.parse('http://10.52.53.134/goodhealth/pesan_obat/update.php?id=$id'),
  );
